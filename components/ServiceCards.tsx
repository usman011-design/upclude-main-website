'use client';

import { useState, useRef, useCallback, useEffect } from 'react';

const AnimatedCard = ({ children }: { children: React.ReactNode }) => {
  const cardRef = useRef<HTMLDivElement>(null);
  const [rotate, setRotate] = useState({ x: 0, y: 0 });
  const [isPressed, setIsPressed] = useState(false);

  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    if (!cardRef.current) return;
    const rect = cardRef.current.getBoundingClientRect();
    setRotate({
      x: (rect.height / 2 - (e.clientY - rect.top)) / 14,
      y: ((e.clientX - rect.left) - rect.width / 2) / 14,
    });
  }, []);

  const handleMouseLeave = useCallback(() => {
    setRotate({ x: 0, y: 0 });
    setIsPressed(false);
  }, []);

  return (
    <div
      onMouseMove={handleMouseMove}
      onMouseLeave={handleMouseLeave}
      onMouseDown={() => setIsPressed(true)}
      onMouseUp={() => setIsPressed(false)}
      style={{ perspective: 900 }}
    >
      <div
        ref={cardRef}
        style={{
          borderRadius: 16,
          cursor: 'pointer',
          transform: `rotateX(${rotate.x}deg) rotateY(${rotate.y}deg) scale(${isPressed ? 0.96 : 1})`,
          transition: 'transform 0.15s ease',
        }}
      >
        {children}
      </div>
    </div>
  );
};

interface Service {
  title: string;
  img: string;
}

const services: Service[] = [
  { title: 'DevOps', img: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Frame%20162684-fWpETLNlBhoK1s2M0goT3xwPygygOd.png' },
  { title: 'Backend Development', img: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Frame%20509%20%281%29-JGtBxEZHvsn0fXKU54YqWMVO8veAFQ.png' },
  { title: 'AI Development', img: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Frame%20509-MlKa8hTVQL8Knw5xqNXU6FIP8msH5k.png' },
  { title: 'UI/UX Design', img: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Frame%20508%20%282%29-MBJZ9Idd8NDikExOrjcby1R0YOxS5e.png' },
  { title: 'Mobile Apps', img: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Frame%20508%20%281%29-P8bUtxfWV5ILKz3bcFwDn9J7AHby5Z.png' },
  { title: 'Web Development', img: 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Frame%20508-Pkv2dH3V6hSroPO19uGK1mWJREsEi5.png' },
];

const CARD_SIZE = 320;
const SPEED = 3;
const SPACING = CARD_SIZE + 140;

const CardFace = ({ service }: { service: Service }) => (
  <div style={{ position: 'relative', width: CARD_SIZE, height: CARD_SIZE }}>
    <img
      src={service.img}
      alt={service.title}
      style={{
        width: '100%',
        height: '100%',
        objectFit: 'cover',
        display: 'block',
        borderRadius: 16,
      }}
    />
    <div style={{
      position: 'absolute',
      bottom: 0,
      left: 0,
      right: 0,
      background: 'rgba(0,0,0,0.9)',
      padding: '14px 16px',
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
      borderBottomLeftRadius: 16,
      borderBottomRightRadius: 16,
    }}>
      <p style={{
        fontFamily: 'system-ui, -apple-system, sans-serif',
        fontSize: 16,
        fontWeight: 700,
        color: '#fff',
        margin: 0,
        textAlign: 'center',
        letterSpacing: '0.01em',
      }}>
        {service.title}
      </p>
    </div>
  </div>
);

export default function ServiceCards() {
  const [vw, setVw] = useState(1440);
  const phaseRef = useRef(0);
  const [phase, setPhase] = useState(0);
  const lastRef = useRef<number | null>(null);
  const rafRef = useRef(0);

  useEffect(() => {
    const measure = () => setVw(window.innerWidth);
    measure();
    window.addEventListener('resize', measure);
    return () => window.removeEventListener('resize', measure);
  }, []);

  useEffect(() => {
    const tick = (ts: number) => {
      if (lastRef.current !== null) {
        const dt = (ts - lastRef.current) / 1000;
        phaseRef.current = (phaseRef.current + SPEED * dt) % 180;
        setPhase(phaseRef.current);
      }
      lastRef.current = ts;
      rafRef.current = requestAnimationFrame(tick);
    };
    rafRef.current = requestAnimationFrame(tick);
    return () => cancelAnimationFrame(rafRef.current);
  }, []);

  // Ellipse radii
  // RX: half the horizontal spread of the arc
  const RX = vw / 2.5 + CARD_SIZE * 0.5;
  // RY: the vertical depth of the ellipse.
  // We want cards at the sides to dip DOWN (below the container bottom edge),
  // and the card at the top of the arc (centre) to sit fully visible.
  // By making RY smaller than before, the arc is flatter and cards stay higher.
  const RY = CARD_SIZE * 1.6;

  const ARC_PX = Math.PI * RX;
  const N = Math.ceil(ARC_PX / SPACING) - 1;
  const STEP = 180 / N;

  // The container only needs to be tall enough to show the highest card (center of arc)
  // Center card sits at y = -RY relative to the bottom anchor.
  // So container height = RY + CARD_SIZE (full card) + some breathing room
  const BOTTOM_PADDING = 60; // how many px of card to peek at the very bottom sides
  const containerHeight = RY + CARD_SIZE + BOTTOM_PADDING;

  const slots = Array.from({ length: N }, (_, i) => {
    const deg = ((i * STEP + phase) % 180 + 180) % 180;
    const rad = (deg * Math.PI) / 180;

    // x: left(-RX) to right(+RX)
    const x = Math.cos(rad) * RX;
    // y: arc goes UP from the bottom anchor. sin(0°)=sin(180°)=0 (sides), sin(90°)=1 (top center)
    const y = Math.sin(rad) * RY;

    // Cards at centre (high up) appear larger; cards at sides appear smaller
    const sinVal = Math.sin(rad);
    const scale = 0.75 + sinVal * 0.25; // range 0.75 → 1.0
    const zIdx = Math.round(sinVal * 100);
    const svcIdx = i % services.length;

    return { x, y, scale, zIdx, svcIdx, key: i };
  });

  const sorted = [...slots].sort((a, b) => a.zIdx - b.zIdx);

  return (
    <div style={{
      position: 'relative',
      width: '100vw',
      height: containerHeight,
      overflow: 'hidden',
      background: 'transparent',
    }}>
      {sorted.map(({ x, y, scale, zIdx, svcIdx, key }) => (
        <div
          key={key}
          style={{
            position: 'absolute',
            left: '50%',
            // Anchor to the bottom of the container.
            // Each card is shifted UP by `y` (arc height) and centered on its x position.
            // We subtract CARD_SIZE so the bottom edge of the card sits on the arc point,
            // meaning the card body is fully above that point.
            bottom: -CARD_SIZE / 2 + BOTTOM_PADDING / 2,
            transform: `translate(calc(${x}px - 50%), calc(${-y}px)) scale(${scale})`,
            zIndex: zIdx,
            willChange: 'transform',
            transformOrigin: 'bottom center',
          }}
        >
          <AnimatedCard>
            <CardFace service={services[svcIdx]} />
          </AnimatedCard>
        </div>
      ))}
    </div>
  );
}